...
\begin{table}[H]
\centering
\caption{Desempenho dos modelos sob condições normais e adversariais}
...
\begin{table}[H]
\centering
\caption{Métricas de desempenho sob ataque adversarial}
...
Para complementar a análise quantitativa, empregou-se a técnica GradCAM para identificar as regiões de atenção dos modelos. As Figuras~\ref{fig:gradcam_original} e~\ref{fig:gradcam_adversarial} ilustram exemplos de imagens originais e suas versões adversariais, destacando alterações nas áreas de interesse (por exemplo, olhos e lábios).

\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{gradcam_original.png}
\includegraphics[width=0.45\textwidth]{gradcam_adversarial.png}
\caption{Exemplos de visualização GradCAM: (a) imagem original; (b) imagem adversarial.}
\label{fig:gradcam}
\end{figure}
