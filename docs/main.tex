...
\begin{itemize}
  \item \textbf{Rede Neural Convolucional (CNN):} Implementada em PyTorch, esta arquitetura
        ...
        camadas totalmente conectadas. Os hiperparâmetros, tais como taxa de aprendizado
        (\texttt{[learning\_rate]}), número de épocas (\texttt{[epochs]}) e tamanho do batch
        (\texttt{[batch\_size]}), foram ajustados para maximizar a acurácia no conjunto de validação.
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \hline
    CNN             & [AC\_N\_CNN]                  & [AC\_A\_CNN]                       & [Redu\_CNN]           \\
    Mini UNET       & [AC\_N\_UNET]                 & [AC\_A\_UNET]                      & [Redu\_UNET]          \\
    XGBoost         & [AC\_N\_XGB]                  & [AC\_A\_XGB]                       & [Redu\_XGB]           \\
    \hline
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \hline
    CNN             & [Prec\_CNN]            & [Rec\_CNN]           & [F1\_CNN]              \\
    Mini UNET       & [Prec\_UNET]           & [Rec\_UNET]          & [F1\_UNET]             \\
    XGBoost         & [Prec\_XGB]            & [Rec\_XGB]           & [F1\_XGB]              \\
    \hline
  \end{tabular}
\end{table}
...
identificar as regiões de atenção dos modelos. As
Figuras~\ref{fig:gradcam_original} e~\ref{fig:gradcam_adversarial} ilustram
exemplos de imagens originais e suas versões adversariais, destacando
alterações nas áreas de interesse (por exemplo, olhos e lábios).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.45\textwidth]{gradcam_original.png}
  \includegraphics[width=0.45\textwidth]{gradcam_adversarial.png}
  \caption{Exemplos de visualização GradCAM: (a) imagem original; (b) imagem adversarial.}\label{fig:gradcam}
\end{figure}
