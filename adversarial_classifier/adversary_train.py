...
from utils import visualize_adversarial_examples, visualize_one_adversarial_example
...
def evaluate(
    generator, classifier, data_loader, device, confusion_matrix=False, verbose=False
):
    generator.eval()
    classifier.eval()
    correct = 0
...
def main():
    ...
    lambda_pixel = exp_config.get("lpixel")
    ...
    image_path = "../data/test/fake/00QKZTHTLF.jpg"

    fake_image = DeepFakeDataset(
        csv_file="../data/test.csv",
        root_dir="../data",
        transform=transform,
        fraction=1,
    ...
    print(classifier(torch.unsqueeze((fake_image + 1) / 2, 0).to(device)))
