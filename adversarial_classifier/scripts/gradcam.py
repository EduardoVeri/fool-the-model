...
def main():
    ...
    # model = UNet()
    # model.load_state_dict(torch.load("../results/unet_classifier/unet_99acc.pth"))
    # model.eval()
    # model.to(device)
    # layer = model.dec2[-1]
    ...
    # for i in range(len(DeepFakeDataset()/2)):
    #     continue

    image_tensor, label = DeepFakeDataset(
    ...
    ).get_image_by_path("../data/test/fake/00QKZTHTLF.jpg")
    ...
    # 4) Run Grad-CAM
    heatmap = gradcam(inv_transform(image_tensor), class_idx=None)
    # If class_idx=None, it uses the predicted label.
    # If you specifically want the "fake" class (say label=1), pass class_idx=1.

    # 5) Visualize
    show_gradcam_on_image(inv_transform(image_tensor.cpu()), heatmap)

    # Do gradcam with the image generated by the generator
    generator = MidTermGenerator(epsilon=0.01)
    generator.load_state_dict(
        torch.load(
            "../results/adversarial/unet_good_perception/e01_allreal_adversary.pth"
        )
    )
    generator.eval()
    generator.to(device)
    ...
    heatmap = gradcam(inv_transform(adv_img), class_idx=None)
    show_gradcam_on_image(inv_transform(adv_img).cpu(), heatmap)
